...
namespace ServerMultiTool.Model.Domain.Pipeline
{
    public class PipelineStep(string name, string description, int order = 0)
    {
        ...
        public List<PipelineOperation> Operations { get; set; } = [];

        public PipelineStep AddOperation(PipelineOperation operation)
        {
            if (operation == null)
        ...
        public PipelineStep RemoveOperation(PipelineOperation operation)
        {
            if (operation == null)
                throw new ArgumentNullException(nameof(operation), "Operation cannot be null.");
